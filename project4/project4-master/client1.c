...
void init() {
  ...
  // get msgq_id1 from shared memory
	msgq_id1 = atoi(shm_data1);
	// open a message queue
	if((msgq_id1 = msgget(msgq_id1, 0666 | IPC_CREAT)) == -1) {
		perror("msgget");
		exit(1);
	}
}
...
void terminate() {
	...
	if(msgctl(msgq_id1, IPC_RMID, NULL) == -1) {
		perror("msgctl");
		exit(1);
	}
	exit(0);
...
int main() {
	...
	while (1) {
		...
		if(msgrcv(msgq_id1, &message, sizeof(message), 0, MSG_NOERROR) == -1) {
			perror("msgrcv");
			exit(1);
		}
